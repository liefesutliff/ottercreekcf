function toggleMenu() {
  const navBar = document.querySelector('.navBar');
  navBar.classList.toggle('showMenu');
}

function textToggle24() {
  var element = document.getElementById('toggle2024Retreat');

  if (element.innerText === '↓ Open 2024 Sessions ↓') {
    element.innerText = '↑ Close 2024 Sessions ↑';
  } else {
    element.innerText = '↓ Open 2024 Sessions ↓';
  }
}

function textToggle23() {
  var element = document.getElementById('toggle2023Retreat');

  if (element.innerText === '↓ Open 2023 Sessions ↓') {
    element.innerText = '↑ Close 2023 Sessions ↑';
  } else {
    element.innerText = '↓ Open 2023 Sessions ↓';
  }
}

let retreat2024ContentVisible  = false; // Flag to track the state of the content

function womensRetreat2024() {
  var divElement = document.getElementsByClassName('retreatVideos2024')[0];

  if (retreat2024ContentVisible) {
    // Clear the div if content is visible
    divElement.innerHTML = '';
    retreat2024ContentVisible = false; // Update the flag
  } else {
    // Inject new content if the div is empty
    divElement.innerHTML = `
      <iframe width="560" height="315" src="https://www.youtube.com/embed/EJvKXbpLDSY?si=PHv64iuDw2yHvRLw" style="position: relative; width: 100%; height: 200%; margin: 0 auto; padding-top: 56.25% top: 0; left: 0;" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
      <iframe width="560" height="315" src="https://www.youtube.com/embed/ktjagTqU4to?si=62cjM5froVOilM6J" style="position: relative; width: 100%; margin: 0 auto; padding-top: 56.25%;" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
      <iframe width="560" height="315" src="https://www.youtube.com/embed/xXdZUW44wPM?si=VRLNeQA4dfdx9nBN" style="position: relative; width: 100%; margin: 0 auto; padding-top: 56.25%;" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
      <iframe width="560" height="315" src="https://www.youtube.com/embed/Hq7PQUkItnM?si=BEOddL5Dzeopw1os" style="position: relative; width: 100%; margin: 0 auto; padding-top: 56.25%;" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
    `;
    retreat2024ContentVisible = true; // Update the flag
  }
}

let retreat2023ContentVisible  = false; // Flag to track the state of the content

function womensRetreat2023() {
  var divElement = document.getElementsByClassName('retreatVideos2023')[0];

  if (retreat2023ContentVisible) {
    // Clear the div if content is visible
    divElement.innerHTML = '';
    retreat2023ContentVisible = false; // Update the flag
  } else {
    // Inject new content if the div is empty
    divElement.innerHTML = `
      <a href="media/WomensRetreat2023Session1.pdf" target="_blank">Download Session 1 PDF</a><br><br>
      <iframe width="560" height="315" src="https://www.youtube.com/embed/5jswixRLqC8?si=3I0AIzdKjJV3-VwS" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe><br><br>
      <a href="media/WomensRetreat2023Session2.pdf" target="_blank">Download Session 2 PDF</a><br><br>
      <iframe width="560" height="315" src="https://www.youtube.com/embed/iqSDAezJ1M4?si=MJAljUE9dqhrSL6s" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe><br><br>
      <a href="media/WomensRetreat2023Session3.pdf" target="_blank">Download Session 3 PDF</a><br><br>
      <iframe width="560" height="315" src="https://www.youtube.com/embed/KSQavE-01pU?si=uR4yQkvkaPNMGNBR" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe><br><br>
      <a href="media/WomensRetreat2023Session4.pdf" target="_blank">Download Session 4 PDF</a><br><br>
      <iframe width="560" height="315" src="https://www.youtube.com/embed/ptKTG3Y_odc?si=0bST-63gb1ifhhkE" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe><br><br>
    `;
    retreat2023ContentVisible = true; // Update the flag
  }
}